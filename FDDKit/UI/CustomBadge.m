...
- (id) initWithString:(NSString *)badgeString withScale:(CGFloat)scale withShining:(BOOL)shining withframe:(CGRect)frame
{
...
- (void) autoBadgeSizeWithString:(NSString *)badgeString
{
	...
	CGSize stringSize = [badgeString commonSizeWithFont:[UIFont systemFontOfSize:12] constrainedToSize:CGSizeMake(100, 100)];
	CGFloat flexSpace;
...
+ (CustomBadge *)customBadgeWithString:(NSString *)badgeString frame:(CGRect)frame
{
    return [[self alloc] initWithString:badgeString withScale:1.0 withShining:YES withframe:frame];
}
...
-(void) drawRoundedRectWithContext:(CGContextRef)context withRect:(CGRect)rect
{
    ...
    if (self.frameShdowFlag) {//yohunl add 20140610 去掉边框的阴影
...
- (void)drawRect:(CGRect)rect {
	...
	if ([self.badgeText length]>0) {
		...
		CGSize textSize = [self.badgeText commonSizeWithFont:textFont];
